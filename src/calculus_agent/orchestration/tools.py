...
def knowledge_tools(context: AgentRunContext) -> list[AgentTool]:
    ...
    def inspect_supply(arguments: dict) -> dict:
        ...
        if grade:
            statement = statement.where(Question.grade == str(grade))
    ...
    return [
        ...
        AgentTool(
            ...
            description="统计指定年级、难度和知识点条件下的题库供给，不生成试卷。",
            parameters={
                "type": "object",
                "properties": {
                    "grade": {"type": "string"},
                    "difficulty_min": {"type": "number", "minimum": 0, "maximum": 1},
                    "difficulty_max": {"type": "number", "minimum": 0, "maximum": 1},
