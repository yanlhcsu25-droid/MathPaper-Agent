...
def _match_questions(
    ...
    if request.grade:
        statement = statement.where(Question.grade == request.grade)
    ...
    for question in questions:
        ...
        if question.difficulty is not None:
            distance = abs(question.difficulty - request.target_difficulty)
            score += max(0, 1 - distance)
            reasons.append(f"难度接近：{question.difficulty:.2f}")
    ...
    *,
    ...
    if reasons is None:
        ...
        if question_type and question.question_type == question_type:
            reasons.append(f"题型相同：{question.question_type}")
        if question.difficulty is not None:
            reasons.append(f"难度：{question.difficulty:.2f}")
